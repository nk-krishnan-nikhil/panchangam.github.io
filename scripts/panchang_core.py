...
def get_naksatra_index(true_lunar_longitude: float) -> int:
    return trunc(true_lunar_longitude * 27 / 360) % 27


def get_naksatra_index_at(ahargana: float) -> int:
    return get_naksatra_index(get_true_lunar_longitude(ahargana))


def find_naksatra_transition_fraction(
    ahargana_local: float, left_frac: float, right_frac: float, left_index: int
) -> float:
    left = left_frac
    right = right_frac
    ...
    for _ in range(40):
        mid = (left + right) / 2
        mid_index = get_naksatra_index_at(ahargana_local + mid)
...
def get_naksatra_segments_for_day(ahargana_local: float, step_minutes: int = 60) -> list[tuple[int, int, int]]:
    day_minutes = 24 * 60
    start_index = get_naksatra_index_at(ahargana_local)
    ...
    while minute <= day_minutes:
        scan_minute = min(minute, day_minutes)
        fraction = scan_minute / day_minutes
        index_now = get_naksatra_index_at(ahargana_local + fraction)

        if index_now != previous_index:
            transition_fraction = find_naksatra_transition_fraction(
                ahargana_local,
                previous_fraction,
                fraction,
                previous_index,
            )
            transition_minute = int(round(transition_fraction * day_minutes))
            transition_minute = max(0, min(day_minutes, transition_minute))

            if transitions and transition_minute <= transitions[-1][0]:
                transitions[-1] = (transitions[-1][0], index_now)
            else:
                transitions.append((transition_minute, index_now))

            previous_index = index_now
    ...
    for transition_minute, next_index in transitions:
        if transition_minute > segment_start:
            segments.append((segment_start, transition_minute, segment_index))
        segment_start = transition_minute
        segment_index = next_index

    if segment_start < day_minutes:
        segments.append((segment_start, day_minutes, segment_index))

    if not segments:
        segments.append((0, day_minutes, start_index))

    return segments


def format_naksatra_segments(segments: list[tuple[int, int, int]], include_malayalam: bool) -> str:
    timeline_parts: list[str] = []

    for start_minute, end_minute, naksatra_index in segments:
        naksatra_name = NAKSHATRA_NAMES[naksatra_index]
        naksatra_name_ml = NAKSHATRA_ML.get(naksatra_name, naksatra_name)
        start_time = format_hhmm(start_minute)
        end_time = "24:00" if end_minute >= 24 * 60 else format_hhmm(end_minute)

        if include_malayalam:
            label = f"{naksatra_name_ml} ({naksatra_name})"
        else:
            label = naksatra_name

        timeline_parts.append(f"{label} {start_time}-{end_time}")
...
def compute_day_panchang(day_date: date, latitude: float, longitude: float) -> PanchangDay:
    ...
    naksatra_segments = get_naksatra_segments_for_day(ahargana_local)
    naksatra_timeline_ml = format_naksatra_segments(naksatra_segments, include_malayalam=True)
    naksatra_timeline_en = format_naksatra_segments(naksatra_segments, include_malayalam=False)
    ...
    desc_lines = [
        ...
        f"നക്ഷത്രം (സൂര്യോദയ സമയത്ത്): {nakshatra_ml} ({nakshatra})",
        f"നക്ഷത്രം (സമയക്രമം): {naksatra_timeline_ml}",
        f"Nakshatra Timeline: {naksatra_timeline_en}",
        f"യോഗം: {yoga}",
        f"കരണം: {karana}",
